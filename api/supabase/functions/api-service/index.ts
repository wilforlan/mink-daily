...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const apiPattern = new URLPattern({ pathname: '/api/:route' })
    const matchingPath = apiPattern.exec(url)
    const route = matchingPath ? matchingPath.pathname.groups.route : null
    ...
    switch (true) {
      ...
      case route === 'tasks' && method === 'PUT':
        return updateTask(supabaseClient, id as string, task)
      case route === 'tasks' && method === 'DELETE':
